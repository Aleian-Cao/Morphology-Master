...
export const LessonFlow: React.FC<LessonFlowProps> = ({
  ...
  const [selectedDerivative, setSelectedDerivative] =
    useState<RichDerivative | null>(null);
  ...
  useEffect(() => {
    const hydrate = async () => {
      if (!lesson.dissectionPack || lesson.dissectionPack.length === 0) {
        const enriched = await enrichLessonData(
          lesson.id,
          lesson.root,
          lesson.category,
          lesson.tier,
  ...
  const handleSandboxCheck = async () => {
    ...
    const result = await verifyUserDerivative(
      lesson.root,
      userWord,
      customApiKey,
    );
  ...
  const startRemediation = async () => {
    ...
    const plan = await generateRemediation(
      lesson.root,
      missedConcepts,
      customApiKey,
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-stone-50">
        ...
        <p className="text-stone-500">
          Generating bespoke lesson for {lesson.root}
        </p>
      </div>
    );
  }
  ...
  const renderHeader = () => (
    <div className="bg-white border-b border-stone-200 p-4 flex justify-between items-center shrink-0 shadow-sm z-10">
      <button
        onClick={onExit}
        className="text-stone-500 hover:text-stone-800 text-sm font-bold uppercase tracking-wider"
      >
        &larr; Exit
      </button>
      ...
      <div className="flex gap-1">
        {["DISCOVERY", "DISSECTION", "DERIVATION", "DRILL"].map((p, i) => {
          ...
          return (
            <div
              key={p}
              className={`h-1.5 w-8 rounded-full transition-colors ${active ? "bg-orange-500" : "bg-stone-200"}`}
            />
          );
        })}
      </div>
  ...
  const renderDiscovery = () => (
    <div className="flex flex-col items-center justify-center space-y-6 animate-fade-in p-4 max-w-2xl mx-auto w-full pb-20">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 border border-stone-100 w-full text-center relative overflow-hidden">
        ...
        <h2 className="text-sm font-bold tracking-widest text-stone-400 uppercase mb-2">
          The Root
        </h2>
        <div className="flex items-center justify-center gap-3 mb-2">
          <h1 className="text-6xl font-serif font-bold text-stone-900">
            {lesson.root}
          </h1>
          <button
            onClick={() => playAudio(lesson.root)}
        ...
        <div className="space-y-2">
          <p className="text-2xl font-serif text-stone-800 font-bold">
            {lesson.meaning}
          </p>
          {bilingual && (
            <p className="text-xl font-serif text-blue-600 font-medium">
              {lesson.meaning_vi}
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 relative">
          ...
          {bilingual && (
            <p className="text-blue-600 text-sm italic">{lesson.funFact_vi}</p>
          )}
        </div>
      </div>
    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row gap-8 items-start h-[calc(100vh-140px)]">
      ...
      <div className="flex-1 w-full flex flex-col h-full">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-stone-200 p-2 flex-grow min-h-[300px] mb-4 overflow-hidden relative">
          <WordTree
            root={lesson.root}
            derivatives={lesson.richDerivatives || []}
            onNodeClick={(word) => {
              const found = lesson.richDerivatives?.find(
                (d) => d.word === word,
              );
              if (found) {
        ...
        <div className="bg-stone-100 p-4 rounded-xl border border-stone-200">
          ...
          {sandboxResult && (
            <div
              className={`mt-2 text-sm p-2 rounded ${sandboxResult.valid ? "bg-green-100 text-green-800" : "bg-red-50 text-red-800"}`}
            >
              {sandboxResult.text}
            </div>
          )}
      ...
      <div className="w-full md:w-80 flex-shrink-0 h-full overflow-y-auto custom-scrollbar">
        {selectedDerivative ? (
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-orange-100 animate-slide-in-right h-full">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-3xl font-serif font-bold text-stone-900">
                {selectedDerivative.word}
              </h3>
              <button
                onClick={() => playAudio(selectedDerivative.word)}
                className="text-orange-500 bg-orange-50 p-2 rounded-full"
              >
                <Volume2 size={20} />
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-stone-800 font-medium">
                  {selectedDerivative.definition}
                </p>
                {bilingual && (
                  <p className="text-blue-600 mt-1">
                    {selectedDerivative.definition_vi}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="bg-stone-50 p-4 rounded-lg border-l-4 border-stone-300">
                  <p className="italic text-stone-600 mb-2">
                    "{selectedDerivative.example}"
                  </p>
                  {bilingual && (
                    <p className="italic text-blue-500 text-sm">
                      "{selectedDerivative.example_vi}"
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="bg-stone-50 border-2 border-dashed border-stone-200 rounded-2xl h-full flex flex-col items-center justify-center text-stone-400 p-8 text-center">
            ...
            <p>
              Select a word from the tree or type one in the sandbox to analyze
              it.
            </p>
          </div>
        )}
      ...
      <div className="w-full max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="bg-stone-200 px-3 py-1 rounded-full text-sm font-mono font-bold">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-stone-200 relative overflow-hidden">
          ...
          <div className="absolute top-0 left-0 h-1 bg-stone-100 w-full">
            <div
              ...
              style={{
                width: `${((currentQuestionIndex + 1) / drillQuestions.length) * 100}%`,
              }}
            ></div>
          </div>

          <h3 className="text-lg font-medium text-stone-800 mb-6 leading-relaxed mt-4">
            {currentQ.question}
          </h3>
          ...
          {showAnswer && (
            <div className="mt-6 pt-6 border-t border-stone-100 animate-fade-in">
              ...
              <div className="flex justify-end">
                ...
                >
                  {currentQuestionIndex === drillQuestions.length - 1
                    ? "Finish & Grade"
                    : "Next"}
                </button>
              </div>
