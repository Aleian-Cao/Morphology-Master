...
export const TierAssessment: React.FC<TierAssessmentProps> = ({
  tierId,
  roots,
  isPro,
  customApiKey,
        ...
        <p key={i} className="mb-2 text-stone-700 leading-relaxed">
          {parts.map((part, j) => {
            if (part.startsWith("**") && part.endsWith("**")) {
              return (
                <span key={j} className="font-bold text-stone-900">
                  {part.slice(2, -2)}
                </span>
              );
            }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-stone-500">
          Consulting the AI for challenging questions.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
        ...
        <p className="text-stone-500 mb-6">
          The AI historians couldn't assemble your exam papers. Please try
          again.
        </p>
        <div className="flex gap-4">
          <button
            onClick={onCancel}
            className="px-6 py-2 rounded-lg text-stone-600 hover:bg-stone-100 font-bold"
          >
            Cancel
          </button>
          <button
  ...
  if (finished) {
    const score = Object.keys(answers).filter(
      (k) => answers[parseInt(k)] === questions[parseInt(k)].correctAnswer,
    ).length;
      ...
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-xl text-center">
        ...
        <div className="flex justify-center gap-8 mb-6">
          ...
          <div>
            ...
            <p
              className={`text-4xl font-bold ${seconds < 30 ? "text-red-500" : "text-stone-800"}`}
            >
              {formatTime(seconds)}
            </p>
          </div>
          ...
          {evaluating ? (
            <div className="flex items-center justify-center py-4">
              ...
              <span className="text-stone-500">
                Analyzing your speed and accuracy...
              </span>
            </div>
          ) : (
            <div className="text-sm">{formatFeedback(evaluation)}</div>
          )}
        </div>

        <p className="text-stone-400 text-sm">
          Saving results and returning...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="bg-white border-b border-stone-200 p-4 shrink-0 flex justify-between items-center px-4 md:px-8 z-10 shadow-sm">
        <div>
          <h1 className="text-2xl font-serif font-bold">
            Tier {tierId} Assessment
          </h1>
        ...
        <div className="flex items-center gap-4">
          <div
            className={`flex items-center gap-2 font-mono text-xl font-bold px-4 py-2 rounded-lg ${seconds > 300 ? "text-red-600 bg-red-50" : "text-stone-700 bg-white shadow-sm"}`}
          >
            <Clock size={20} />
            {formatTime(seconds)}
          </div>
          <button
            onClick={onCancel}
            className="text-stone-500 underline text-sm"
          >
            Cancel
          </button>
        </div>
            ...
            {questions.map((q, idx) => (
              ...
              >
                <div className="flex gap-4 mb-4">
                  <span className="bg-stone-900 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold flex-shrink-0">
                    {idx + 1}
                  </span>
